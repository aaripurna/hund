%% Copied from esaml https://hex.pm/packages/esaml
...
canon_name(Ns, Name, Nsp) ->
  NsPartRaw =
    case Ns of
      ...
      [] ->
        if
          Nsp == [] -> 'urn:oasis:names:tc:SAML:2.0:assertion';
          true -> Nsp#xmlNamespace.default
        end;

      _ ->
        case proplists:get_value(Ns, Nsp#xmlNamespace.nodes) of
          undefined -> error({ns_not_found, Ns, Nsp});
          Uri -> atom_to_list(Uri)
        end
    end,
  NsPart =
    if
      is_atom(NsPartRaw) -> atom_to_list(NsPartRaw);
      true -> NsPartRaw
    end,
  NamePart =
    if
...
attr_lte(AttrA, AttrB) ->
  ...
  B = canon_name(AttrB),
  PrefixedA =
    case AttrA#xmlAttribute.nsinfo of
      {_, _} -> true;
      _ -> false
    end,
  PrefixedB =
    case AttrB#xmlAttribute.nsinfo of
      {_, _} -> true;
      _ -> false
    end,
  if
...
xml_safe_string(Term, Quotes) -> xml_safe_string(io_lib:format("~p", [Term]), Quotes).
...
c14n(
  #xmlAttribute{nsinfo = NsInfo, name = Name, value = Value},
  _KnownNs,
  ActiveNS,
  _Comments,
  _InclNs,
  ...
  case NsInfo of
    ...
    _ -> ["\"", xml_safe_string(Value, true), "=\"", atom_to_list(Name), " " | Acc]
  end;
...
c14n(Elem = #xmlElement{}, KnownNSIn, ActiveNSIn, Comments, InclNs, Acc) ->
  ...
  Default =
    case Elem#xmlElement.nsinfo of
      ...
      _ ->
        % omit a default namespace if it is not visibly utilized.
        []
    end,
  {ActiveNS, ParentDefault} =
    case ActiveNSIn of
  ...
  Acc1 =
    case Elem#xmlElement.nsinfo of
      ...
      _ -> [atom_to_list(Elem#xmlElement.name), "<" | Acc]
    end,
  % xmlns definitions
  {Acc2, FinalActiveNS} =
    if
      ...
      not (Default =:= []) -> {Acc1, [{default, Default} | NewActiveNS]};
    ...
    lists:foldl(
      fun
        (Ns, AccIn) ->
          [
            "\"",
            xml_safe_string(proplists:get_value(Ns, KnownNS, ""), true),
            "=\"",
            Ns,
            ":",
            " xmlns" | AccIn
  ...
  Acc4 =
    lists:foldl(
      fun (Attr, AccIn) -> c14n(Attr, KnownNS, FinalActiveNS, Comments, InclNs, AccIn) end,
      Acc3,
      Attrs
    ),
  ...
  Acc6 =
    lists:foldl(
      fun (Kid, AccIn) -> c14n(Kid, KnownNS, FinalActiveNS, Comments, InclNs, AccIn) end,
      Acc5,
      Elem#xmlElement.content
    ),
  % and finally add the close tag
  case Elem#xmlElement.nsinfo of
    {Ns, Name} -> [">", Name, ":", Ns, "</" | Acc6];
    _ -> [">", atom_to_list(Elem#xmlElement.name), "</" | Acc6]
  end;

% I do not give a shit
c14n(_, _KnownNS, _ActiveNS, _Comments, _InclNs, Acc) -> Acc.

%% @doc Puts an XML document or element into canonical form, as a string.

-spec c14n(XmlThing :: xml_thing()) -> string().
c14n(Elem) -> c14n(Elem, true).
...
c14n(Elem, Comments) -> c14n(Elem, Comments, []).
...
needed_ns_test() ->
  ...
  E1 =
    hund_xml:build_nsinfo(
      Ns,
      #xmlElement{name = 'foo:Blah', attributes = [#xmlAttribute{name = 'bar:name', value = "foo"}]}
    ),
  ["bar", "foo"] = lists:sort(needed_ns(E1, [])),
  E2 =
    hund_xml:build_nsinfo(
      Ns,
      #xmlElement{name = 'Blah', attributes = [#xmlAttribute{name = 'bar:name', value = "foo"}]}
    ),
  ["bar"] = needed_ns(E2, []),
  E3 =
    hund_xml:build_nsinfo(
      ...
      #xmlElement{
        name = 'Blah',
        attributes = [#xmlAttribute{name = name, value = "foo"}],
        content = [#xmlElement{name = 'foo:InnerBlah'}]
      }
    ),
  [] = needed_ns(E3, []),
  E4 = hund_xml:build_nsinfo(Ns, #xmlElement{name = 'Blah'}),
  [] = needed_ns(E4, []),
  [] = needed_ns(E4, ["foo"]),
  {E5, []} =
    xmerl_scan:string(
...
c14n_3_4_test() ->
  ...
  ?assertEqual(Target, c14n(Doc, true)).
...
omit_default_ns_test() ->
  {Doc, _} =
    xmerl_scan:string(
      "<foo:a xmlns:foo=\"urn:foo\"><bar:b xmlns=\"urn:bar\" xmlns:bar=\"urn:bar\"><bar:c /></bar:b></foo:a>",
      [{namespace_conformant, true}]
    ),
  Target =
    "<foo:a xmlns:foo=\"urn:foo\"><bar:b xmlns:bar=\"urn:bar\"><bar:c></bar:c></bar:b></foo:a>",
  Target = c14n(Doc, true).
